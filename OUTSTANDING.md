...
## THE MEASURED DEBTS — read off `corpus.js` at v5.83.0
...
| ~~nights under the **35% narration floor**~~ | **0** | **PASS 4 CLOSED at v5.83.0** — `a2-chassis` and `a3-stone` excluded by ruling |
| spoken blocks — **THE BANTER FLOOR** | **4,656** | must never fall |
| narration blocks | 2,422 | **34.2%** book-wide, **median night 35.6%** |
...
**PASS 4 IS PRICED, AND THE PRICE IS THE POINT.** 27 nights sit under 35% and the full bill
is **570 blocks** — but **174 of those are `a2-chassis`**, a PINNED night that pass 5 takes
past 50% anyway. Doing it in pass 4 would be work thrown away twice, so **pass 4 skips it
...
`a2-housing` 24.2 → 35.8 · `a2-seven` 26.0 → 36.4.

**THE ORDER FROM HERE, worst first:** `a3-for` 18 · `a2-three` 15 · `a2-mirren` 14 ·
`a2-choir` 14 · `a3-granny` 13 · `a2-hand` 12 · `a1-fare` 11 · `a3-voice` 10 ·
`a3-dreamers` 10 · then nines and under.

**NOT EVERY NEW DRIFT FIRE IS A FAULT.** Three of my own blocks fired predicates this pass.
Two were real — a mind-and-body sentence read as `breach`, and *"inside one **closed
hand**"* moved the shard, because `closed hand` is a SHARD_TO_HAND phrase and the pocket
trap wears more than one glove. **The engine's model of where she is should be moved by the
author's staging, never by a narrator reaching for an image of privacy.** The third was
kept: narration naming Three, on Three's own card, in a room she is in. **The test is
whether the sentence meant to do it.**
...
every insert shifts the ones behind it. The second batch on night 18 was written against the
first batch's numbers and the tool refused it — found the needle three and five times rather
...
**THE SEAM REVEAL BUG — FIXED AT v5.92.0.** `playAuthored` added `shown` inside its own
render loop, so no seam in the game had ever revealed at all and reading pace had never
touched one. It gets the same paced walk `showScene` uses, and the rail waits for it.
Proved by sweep row (6g8), which drives the real function and is proved against the shipped
shape. Driven at three paces over a landing, a close and a premiere, plus one live night.
